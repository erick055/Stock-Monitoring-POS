...
$customerReturns = [
    ['item' => 'Tire (Harap)','order' => 'Order: ORD-001','quantity' => 'Qty: 1','reason' => 'Reason: Defective','amount' => 'P 1,500.00','status' => 'Approved','action' => 'Review'],
];
$damageLogs = [
    ['item' => 'Air Filter','note' => 'Water Damaged','quantity' => '3 Units','date' => '2026-07-11','replacement' => 'Replacement Status: Ordered','action' => 'Update'],
];
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Staff Return & Damage | MotoSync</title>
...
<div class="dashboard-shell returns-shell">
    ...
    <main class="dashboard-main returns-main">
        <header class="returns-header">
            ...
            <div><p class="welcome">HANDLE RETURNS, DAMAGES, AND REFUNDS</p><h1>Return &amp; Damage Management</h1><p>Manage return requests, damaged goods, and replacement tracking.</p></div>
        </header>
            ...
            <div class="case-list">
                @foreach($customerReturns as $return)
                    <article class="case-card">
                        <div class="case-main"><strong>{{ $return['item'] }}</strong><small>{{ $return['order'] }}</small><div class="case-meta"><span>{{ $return['quantity'] }}</span><span>{{ $return['reason'] }}</span></div></div>
                        <div class="case-side"><strong class="amount">{{ $return['amount'] }}</strong><span class="status approved">{{ $return['status'] }}</span><button type="button" class="mini-button">{{ $return['action'] }}</button></div>
                    </article>
                @endforeach
            </div>
        </section>
        ...
        <section class="panel returns-panel">
            ...
            <div class="case-list">
                @foreach($damageLogs as $log)
                    <article class="case-card">
                        <div class="case-main"><strong>{{ $log['item'] }}</strong><small>{{ $log['note'] }}</small><div class="case-meta"><span>{{ $log['replacement'] }}</span></div></div>
                        <div class="case-side"><strong>{{ $log['quantity'] }}</strong><small>{{ $log['date'] }}</small><button type="button" class="mini-button success">{{ $log['action'] }}</button></div>
                    </article>
                @endforeach
            </div>
        </section>
