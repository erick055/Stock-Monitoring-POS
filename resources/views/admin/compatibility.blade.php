...
<head>
    ...
    <title>Part Compatibility | MotoSync</title>
...
<div class="dashboard-shell compatibility-shell">
    <aside class="sidebar" data-sidebar>
        ...
        <nav class="nav-list" aria-label="Administrator navigation">
            @foreach($navigation as $index => $item)
                <a class="nav-link {{ $index === 8 ? 'active' : '' }}" href="{{ $item[2] }}"><span>{{ $item[0] }}</span><span>{{ $item[1] }}</span></a>
            @endforeach
        </nav>
    ...
    <main class="dashboard-main compatibility-main">
        <header class="compatibility-header">
            ...
            <div>
                <p class="welcome">SMART PART SUGGESTION AND MAINTENANCE TRACKING</p>
                <h1>Part Compatibility</h1>
                <p>Match products by motorcycle model and track recommended maintenance parts.</p>
            </div>
        </header>

        <section class="panel search-panel">
            <div class="section-heading">
                <div><span class="section-kicker">MODEL SEARCH</span><h2>Search by Motorcycle Model</h2></div>
            </div>
        ...
        <section class="compatibility-grid">
            <section class="panel compatibility-panel">
                <div class="section-heading">
                    <div><span class="section-kicker">MATCHED PRODUCTS</span><h2>Compatible Parts for Honda Click v3(2024)</h2></div>
                </div>
            ...
            <section class="panel compatibility-panel">
                <div class="section-heading">
                    <div><span class="section-kicker">SERVICE PLAN</span><h2>Suggested Maintenance Schedule</h2></div>
                </div>
        ...
        <section class="panel history-panel">
            <div class="section-heading">
                <div><span class="section-kicker">CUSTOMER RECORDS</span><h2>Customer Maintenance History</h2></div>
                <button class="apply-button" type="button" data-refresh-compatibility>Refresh</button>
            </div>
            <div class="history-list">
                @foreach($history as $item)
                    <article class="history-card">
                        <div><strong>{{ $item[0] }}</strong><small>{{ $item[1] }}</small></div>
                        <div><small>{{ $item[2] }}</small><strong>{{ $item[3] }}</strong></div>
                    </article>
                @endforeach
            </div>
        </section>
